...
Future<void> downloadChapter(
  ...
  final mangaMainDirectory = await storageProvider.getMangaMainDirectory(chapter);
  ...
  Future<void> processConvert() async {
    if (ref.watch(saveAsCBZArchiveStateProvider)) {
      await ref.watch(convertToCBZProvider(
              chapterDirectory.path, mangaMainDirectory!.path, chapter.name!, pageUrls.map((e) => e.url).toList())
          .future);
    }
  }
  ...
  Future<void> setProgress(DownloadProgress progress) async {
    ...
    if (download == null) {
      final download = Download(
        id: chapter.id,
        succeeded: progress.completed == 0 ? 0 : (progress.completed / progress.total * 100).toInt(),
        failed: 0,
        total: 100,
  ...
  if (pageUrls.isNotEmpty) {
    bool cbzFileExist = await File(p.join(mangaMainDirectory!.path, "${chapter.name}.cbz")).exists() &&
        ref.watch(saveAsCBZArchiveStateProvider);
    bool mp4FileExist = await File(p.join(mangaMainDirectory.path, "$chapterName.mp4")).exists();
    bool htmlFileExist = await File(p.join(mangaMainDirectory.path, "$chapterName.html")).exists();
    if (!cbzFileExist && itemType == ItemType.manga ||
      ...
      for (var index = 0; index < pageUrls.length; index++) {
        ...
        if (itemType == ItemType.manga) {
          final file = File(p.join(chapterDirectory.path, "${padIndex(index + 1)}.jpg"));
          if (!file.existsSync()) {
            pages.add(PageUrl(
              page.url.trim().trimLeft().trimRight(),
              headers: pageHeaders,
              fileName: p.join(chapterDirectory.path, "${padIndex(index + 1)}.jpg"),
            ));
          }
        } else if (itemType == ItemType.anime) {
          final file = File(p.join(mangaMainDirectory.path, "$chapterName.mp4"));
          if (!file.existsSync()) {
            pages.add(PageUrl(
              page.url.trim().trimLeft().trimRight(),
              headers: pageHeaders,
