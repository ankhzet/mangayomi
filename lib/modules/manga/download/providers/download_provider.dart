...
Future<List<PageUrl>> downloadChapter(
  ...
  if (pageUrls.isNotEmpty) {
    ...
    bool htmlFileExist = await File(p.join(mangaDir.path, "$chapterName.html")).exists();
    if (!cbzFileExist && itemType == ItemType.manga ||
      ...
      for (var index = 0; index < pageUrls.length; index++) {
        ...
        if (itemType == ItemType.manga) {
          ...
          final file = File(p.join(tempDir.path, "Mangayomi", finalPath, "$chapterName.html"));
          if (file.existsSync()) {
            ...
            if (!(await File(p.join(path.path, "$chapterName.html")).exists())) {
              tasks.add(DownloadTask(
                  taskId: page.url,
                  headers: pageHeaders,
                  url: page.url.trim().trimLeft().trimRight(),
    ...
    if (tasks.isEmpty && pageUrls.isNotEmpty) {
      ...
      await FileDownloader().downloadBatch(
        ...
        batchProgressCallback: (succeeded, failed) async {
          if (itemType == ItemType.manga || itemType == ItemType.novel || hasM3U8File) {
