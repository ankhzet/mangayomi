...
class SyncServer extends _$SyncServer {
  ...
  Map<String, dynamic> _getData() {
    ...
    final categories = isar.categorys.filter().idIsNotNull().findAllSync().map((e) => e.toJson()).toList();
  ...
  void _restore(Map<String, dynamic> backup) {
    if (backup['version'] == "1") {
      try {
        ...
        final track = (backup["tracks"] as List?)?.map((e) => Track.fromJson(e)).toList();
        final history = (backup["history"] as List?)?.map((e) => History.fromJson(e)).toList();
        final settingsList = (backup["settings"] as List?)?.map((e) => Settings.fromJson(e)).toList();
        ...
        isar.writeTxnSync(() {
          ...
          isar.settings.first = isar.settings.first
