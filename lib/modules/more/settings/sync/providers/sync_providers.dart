...
class Synching extends _$Synching {
  ...
  List<ChangedPart> getChangedParts(List<ActionType> actionTypes) {
    var query = isar.changedParts.filter().idIsNotNull().and().actionTypeEqualTo(actionTypes.first);
  ...
  void addChangedPart(ActionType action, int? isarId, Object data, bool writeTxn) {
    if (!state.syncOn) {
      return;
    }
    final changedPart = isar.changedParts.filter().actionTypeEqualTo(action).isarIdEqualTo(isarId).findFirstSync();
    if (writeTxn) {
      isar.writeTxnSync(() {
        if (changedPart != null) {
  ...
  Future<void> addChangedPartAsync(ActionType action, int? isarId, Object data, bool writeTxn) async {
    if (!state.syncOn) {
      return;
    }
    final changedPart = isar.changedParts.filter().actionTypeEqualTo(action).isarIdEqualTo(isarId).findFirstSync();
  ...
  void clearChangedParts(List<ActionType> actions) {
    var temp = isar.changedParts.filter().idIsNotNull().and().actionTypeEqualTo(actions.first);
