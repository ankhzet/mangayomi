...
class _LibraryScreenState extends ConsumerState<LibraryScreen>
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            Icon(
              type == 1 ? Icons.check_box : Icons.check_box_outline_blank,
              color: type == 1 ? Theme.of(context).colorScheme.primary : null,
            ),
          ],
        ),
      ),
                        ...
                        return Consumer(
                          builder: (context, ref, child) {
                            ...
                            final unreadChapter = ref.watch(
                              libraryUnreadChaptersStateProvider(
                                itemType: widget.itemType,
                                settings: settings,
                              ),
                            );
                            ...
                            final catIndex = withoutCategory.isNotEmpty
                                ? _tabIndex - 1
                                : _tabIndex;
                            final categoryId = catIndex < 0
                                ? null
                                : categories[catIndex].id!;
  ...
  Iterable<Manga> _filterAndSortManga({
    ...
    return filtered.sorted(switch (sortType) {
      0 => (() {
        final cache = {};
      ...
      2 =>
        (a, b) =>
            multiplier * (a.lastUpdate?.compareTo(b.lastUpdate ?? 0) ?? 0),
      3 =>
        (a, b) =>
            multiplier *
            a.chapters
                ...
                .length
                .compareTo(
                  b.chapters.where((element) => !element.isRead!).length,
                ),
      4 =>
        (a, b) => multiplier * a.chapters.length.compareTo(b.chapters.length),
      5 =>
        (a, b) =>
            multiplier *
            (a.chapters.lastOrNull?.dateUpload?.compareTo(
                  b.chapters.lastOrNull?.dateUpload ?? "",
                ) ??
                0),
      6 =>
        (a, b) => multiplier * (a.dateAdded?.compareTo(b.dateAdded ?? 0) ?? 0),
                              ...
                              isar.writeTxnSync(() async {
                                for (var manga in mangasList) {
                                  if (manga.isLocalArchive ?? false) {
                                    ...
                                    final mangaDir = await _storageProvider
                                        .getMangaMainDirectory(firstChapter);

                                    for (var chapter in manga.chapters) {
                                      final path = await _storageProvider
