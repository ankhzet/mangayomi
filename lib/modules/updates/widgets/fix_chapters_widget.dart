...
class ChaptersFix extends ConsumerStatefulWidget {
  ...
  const ChaptersFix({super.key, required this.update});
...
class _ChaptersFixState extends ConsumerState<ChaptersFix> {
    ...
    try {
      ...
      await isar.writeTxn(() async {
        await isar.updates
            .where()
            .filter()
            .chapter((q) => q.oneOf(ids, (q, id) => q.idEqualTo(id)))
            .deleteAll();
        await isar.historys
            .where()
            .filter()
            .anyOf(ids, (q, id) => q.chapterIdEqualTo(id))
            .deleteAll();
        await isar.viewQueueItems
            .where()
            .anyOf(ids, (q, id) => q.chapterIdEqualTo(id))
            .deleteAll();
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      stream: isar.chapters
          .where()
          .mangaIdEqualTo(manga.id)
          .watch(fireImmediately: true),
      builder: (context, snapshot) {
        final List<Chapter> chapters = snapshot.hasData ? snapshot.data! : [];
        final List<Chapter> duplicates = manga.getDuplicateChapters(
          all: chapters,
        );
        final List<Chapter> unread = manga.getUnreadChapters(
          update.state,
          all: chapters,
        );
        final List<Chapter> ghosts =
            chapters
                .where(
                  (chapter) => chapter.name == null || chapter.name!.isEmpty,
                )
                .toList();
            ...
            child: IconButton(
              ...
              onPressed: () {
                ...
                if (readUpdates > 0) {
                  _deleteChapters(
                    update.state.where(
                      (chapter) => !unread.any((item) => item.id == chapter.id),
                    ),
                  );
                }
              },
              icon: Tooltip(
                message: [
                  duplicates.isNotEmpty
                      ? '${duplicates.length} duplicate chapters'
                      : '',
                  ghosts.isNotEmpty
                      ? '${ghosts.length} chapters with no name'
                      : '',
                  readUpdates > 0
                      ? '$readUpdates updates not removed after reading'
                      : '',
  ...
  return Stack(
    children: [
      Align(
        ...
        child: Icon(size: 12, Icons.medical_services_outlined, color: color),
      ),
      Align(
        alignment: Alignment.center,
        child: SizedBox(
