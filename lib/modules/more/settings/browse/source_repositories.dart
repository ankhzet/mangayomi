...
class _SourceRepositoriesState extends ConsumerState<SourceRepositories> {
  ...
  Widget build(BuildContext context) {
    ...
    final repositories = ref.watch(extensionsRepoStateProvider(widget.itemType));
                                ...
                                TextButton(
                                    onPressed: controller.text.isEmpty || !controller.text.endsWith(".json")
                                        ...
                                        : () async {
                                            try {
                                              final mangaRepos =
                                                  ref.read(extensionsRepoStateProvider(widget.itemType)).toList();
                                              final repo =
                                                  await ref.read(getRepoInfosProvider(jsonUrl: controller.text).future);
                                              mangaRepos.add(repo);
                                              ref
                                                  .read(extensionsRepoStateProvider(widget.itemType).notifier)
                                                  .set(mangaRepos);
                                              ref.invalidate(extensionsRepoStateProvider(widget.itemType));
                                            } catch (e) {
                                              botToast(e.toString());
                                            }
                                    ...
                                    child: Text(
                                      l10n.add,
                                      style: TextStyle(
                                          color: controller.text.isEmpty || !controller.text.endsWith(".json")
